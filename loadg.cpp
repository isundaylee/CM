...
SDL_Surface *LoadGraph(std::string x)
{
    SDL_Surface *ts = IMG_Load(x.c_str()); 
    // SDL_SetColorKey(ts, SDL_TRUE, SDL_MapRGB(ts->format, 9*16+9, 255, 255)); 
    return ts; 
}

void GetGraphSize(SDL_Surface *s, int *x, int *y)
{
    if (s == 0)
        return; 
    *x = s->w; 
...
SDL_Surface *DerivationGraph(int x, int y, int w, int h, SDL_Surface *graph)
{
    // SDL_Surface *s = SDL_CreateRGBSurface(graph->flags, w, h, graph->format->BitsPerPixel, graph->format->Rmask, graph->format->Gmask, graph->format->Bmask, graph->format->Amask); 
    ...
    dest.w = w; 
    dest.h = h; 
    SDL_BlitSurface(graph, &rect, s, &dest); 
    SDL_SetColorKey(s, SDL_TRUE, SDL_MapRGB(s->format, 153, 255, 255)); 
    return s; 
}
